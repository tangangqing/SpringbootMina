...
public class HttpUtils {
	...
	private static String sendHttpRequest(HttpUriRequest request, String method) {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == HTTP_OK) {
				...
				log.debug(MessageFormat.format("HTTP {1} response {0} ", result, method));
			} else {
				log.debug(MessageFormat.format("HTTP {1} response code[{0}] ",
	...
	public static String sendHttpGetRequest(String url) {
		...
		log.debug(MessageFormat.format("send HTTP GET reqeust to [{0}]", url));
	...
	public static String sendHttpPutRequest(String url, String body) {
		...
		try {
			...
			log.debug(MessageFormat.format("send HTTP PUT reqeust to [{0}],content[{1}]", url, body));
	...
	public static String sendHttpDeleteRequest(String url) {
		...
		log.debug(MessageFormat.format("send HTTP DELETE reqeust to [{0}],content[{1}]", url));
